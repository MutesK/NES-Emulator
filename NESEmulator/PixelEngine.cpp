...
namespace PixelEngine
{
    ...
    Sprite::Sprite(std::string sImageFile, PixelEngine::ResourcePack* pack)
    {
    ...
    {
        if (ProgramCounterolData) delete[] ProgramCounterolData;
        width = w;        height = h;
        ProgramCounterolData = new Pixel[width * height];
        for (int32_t i = 0; i < width * height; i++)
            ProgramCounterolData[i] = Pixel();
    }
    ...
    PixelEngine::rcode Sprite::LoadFromPGESprFile(std::string sImageFile, PixelEngine::ResourcePack* pack)
    {
        if (ProgramCounterolData) delete[] ProgramCounterolData;
        ...
        auto ReadData = [&](std::istream& is)
        {
            ...
            ProgramCounterolData = new Pixel[width * height];
            is.read((char*)ProgramCounterolData, width * height * sizeof(uint32_t));
        };

        // These are essentially Memory Surfaces represented by PixelEngine::Sprite
        ...
        if (pack == nullptr)
        {
            ...
            {
                ReadData(ifs);
                return PixelEngine::OK;
            }
            else
                return PixelEngine::FAIL;
        }
        else
        {
    ...
    PixelEngine::rcode Sprite::SaveToPGESprFile(std::string sImageFile)
    {
        if (ProgramCounterolData == nullptr) return PixelEngine::FAIL;
        ...
        if (ofs.is_open())
        {
            ...
            ofs.write((char*)ProgramCounterolData, width * height * sizeof(uint32_t));
            ofs.close();
            return PixelEngine::OK;
        }
    ...
    PixelEngine::rcode Sprite::LoadFromFile(std::string sImageFile, PixelEngine::ResourcePack* pack)
    {
        UNUSED(pack);
#if defined(_WIN32)
        // Use GDI+
        ...
        if (!f) return PixelEngine::NO_FILE;
    ...
    void Sprite::SetSampleMode(PixelEngine::Sprite::Mode mode)
    {
        modeSample = mode;
    }
    ...
    {
        if (modeSample == PixelEngine::Sprite::Mode::NORMAL)
        {
            if (x >= 0 && x < width && y >= 0 && y < height)
                return ProgramCounterolData[y * width + x];
            else
                return Pixel(0, 0, 0, 0);
        }
        else
        {
            return ProgramCounterolData[abs(y % height) * width + abs(x % width)];
        }
    }
    ...
    bool Sprite::SetPixel(int32_t x, int32_t y, Pixel p)
    {
        ...
        if (x >= 0 && x < width && y >= 0 && y < height)
        {
            ProgramCounterolData[y * width + x] = p;
            return true;
        }
        else
            return false;
    ...
    Pixel Sprite::SampleBL(float u, float v)
    {
        ...
        PixelEngine::Pixel p1 = GetPixel(std::max(x, 0), std::max(y, 0));
        PixelEngine::Pixel p2 = GetPixel(std::min(x + 1, (int)width - 1), std::max(y, 0));
        PixelEngine::Pixel p3 = GetPixel(std::max(x, 0), std::min(y + 1, (int)height - 1));
        PixelEngine::Pixel p4 = GetPixel(std::min(x + 1, (int)width - 1), std::min(y + 1, (int)height - 1));

        return PixelEngine::Pixel(
            (uint8_t)((p1.r * u_opposite + p2.r * u_ratio) * v_opposite + (p3.r * u_opposite + p4.r * u_ratio) * v_ratio),
            (uint8_t)((p1.g * u_opposite + p2.g * u_ratio) * v_opposite + (p3.g * u_opposite + p4.g * u_ratio) * v_ratio),
            (uint8_t)((p1.b * u_opposite + p2.b * u_ratio) * v_opposite + (p3.b * u_opposite + p4.b * u_ratio) * v_ratio));
    }

    Pixel* Sprite::GetData() { return ProgramCounterolData; }
    ...
    PixelEngine::rcode ResourcePack::AddToPack(std::string sFile)
    {
        std::ifstream ifs(sFile, std::ifstream::binary);
        if (!ifs.is_open()) return PixelEngine::FAIL;
    ...
    PixelEngine::rcode ResourcePack::SavePack(std::string sFile)
    {
        std::ofstream ofs(sFile, std::ofstream::binary);
        if (!ofs.is_open()) return PixelEngine::FAIL;
    ...
    PixelEngine::rcode ResourcePack::LoadPack(std::string sFile)
    {
        std::ifstream ifs(sFile, std::ifstream::binary);
        if (!ifs.is_open()) return PixelEngine::FAIL;
    ...
    PixelEngine::ResourcePack::sEntry ResourcePack::GetStreamBuffer(std::string sFile)
    {
        return mapFiles[sFile];
    }

    PixelEngine::rcode ResourcePack::ClearPack()
    {
        for (auto& e : mapFiles)
        {
    ...
    {
        sAppName = "Undefined";
        PixelEngine::PGEX::pge = this;
    }

    PixelEngine::rcode PixelGameEngine::Construct(uint32_t screen_w, uint32_t screen_h, uint32_t pixel_w, uint32_t pixel_h, bool full_screen, bool vsync)
    {
        ...
        PixelEngine_ConstructFontSheet();
...
#if defined(__linux__)
        glXSwapBuffers(PixelEngine_Display, PixelEngine_Window);
#endif

        glClear(GL_COLOR_BUFFER_BIT);
        PixelEngine_UpdateViewport();
    }

    PixelEngine::rcode PixelGameEngine::Start()
    {
        // Construct the window
        if (!PixelEngine_WindowCreate())
            return PixelEngine::FAIL;
    ...
    void PixelGameEngine::Clear(Pixel p)
    {
        ...
        for (int i = 0; i < pixels; i++)
            m[i] = p;
#ifdef PixelEngine_DBG_OVERDRAW
        PixelEngine::Sprite::nOverdrawCount += pixels;
#endif
    }
    ...
    void PixelGameEngine::SetPixelMode(std::function<PixelEngine::Pixel(const int x, const int y, const PixelEngine::Pixel&, const PixelEngine::Pixel&)> pixelMode)
    {
    ...
    void PixelGameEngine::PixelEngine_UpdateViewport()
    {
    ...
    void PixelGameEngine::PixelEngine_UpdateWindowSize(int32_t x, int32_t y)
    {
        nWindowWidth = x;
        nWindowHeight = y;
        PixelEngine_UpdateViewport();
    ...
    void PixelGameEngine::PixelEngine_UpdateMouseWheel(int32_t delta)
    {
        nMouseWheelDeltaCache += delta;
    }

    void PixelGameEngine::PixelEngine_UpdateMouse(int32_t x, int32_t y)
    {
    ...
    void PixelGameEngine::EngineThread()
    {
        ...
        while (bAtomActive)
        {
            ...
            while (bAtomActive)
            {
                ...
                XEvent xev;
                while (XPending(PixelEngine_Display))
                {
                    XNextEvent(PixelEngine_Display, &xev);
                    if (xev.type == Expose)
                    {
                        XWindowAttributes gwa;
                        XGetWindowAttributes(PixelEngine_Display, PixelEngine_Window, &gwa);
                    ...
                    else if (xev.type == ButtonPress)
                    {
                        ...
                        case 4:    PixelEngine_UpdateMouseWheel(120); break;
                        case 5:    PixelEngine_UpdateMouseWheel(-120); break;
                        default: break;
                        }
                    }
                    ...
                    {
                        PixelEngine_UpdateMouse(xev.xmotion.x, xev.xmotion.y);
                    }
...
#ifdef PixelEngine_DBG_OVERDRAW
                PixelEngine::Sprite::nOverdrawCount = 0;
#endif
...
#if defined(_WIN32)
#ifdef UNICODE
                    SetWindowText(PixelEngine_hWnd, ConvertS2W(sTitle).c_str());
#else
                    SetWindowText(PixelEngine_hWnd, sTitle.c_str());
#endif
#endif

#if defined (__linux__)
                    XStoreName(PixelEngine_Display, PixelEngine_Window, sTitle.c_str());
#endif
                    nFrameCount = 0;
                }
            }
        ...
        PostMessage(PixelEngine_hWnd, WM_DESTROY, 0, 0);
#endif

#if defined (__linux__)
        glXMakeCurrent(PixelEngine_Display, None, NULL);
        glXDestroyContext(PixelEngine_Display, glDeviceContext);
        XDestroyWindow(PixelEngine_Display, PixelEngine_Window);
        XCloseDisplay(PixelEngine_Display);
#endif
    ...
    void PixelGameEngine::PixelEngine_ConstructFontSheet()
    {
        std::string data;
        ...
        fontSprite = new PixelEngine::Sprite(128, 48);
        int px = 0, py = 0;
        for (int b = 0; b < 1024; b += 4)
        {
            ...
            {
                int k = r & (1 << i) ? 255 : 0;
                fontSprite->SetPixel(px, py, PixelEngine::Pixel(k, k, k, k));
                if (++py == 48) { px++; py = 0; }
            }
        }
    }
...
#if defined(_WIN32)
    HWND PixelGameEngine::PixelEngine_WindowCreate()
    {
        WNDCLASS wc;
        ...
        wc.lpfnWndProc = PixelEngine_WindowEvent;
        wc.cbClsExtra = 0;
        wc.cbWndExtra = 0;
        wc.lpszMenuName = nullptr;
        wc.hbrBackground = nullptr;
#ifdef UNICODE
        wc.lpszClassName = L"PixelEngine_PIXEL_GAME_ENGINE";
#else
        wc.lpszClassName = "PixelEngine_PIXEL_GAME_ENGINE";
#endif
        ...
        if (bFullScreen)
        {
            ...
            nCosmeticOffset = 0;
            HMONITOR hmon = MonitorFromWindow(PixelEngine_hWnd, MONITOR_DEFAULTTONEAREST);
...
#ifdef UNICODE
        PixelEngine_hWnd = CreateWindowEx(dwExStyle, L"PixelEngine_PIXEL_GAME_ENGINE", L"", dwStyle,
            nCosmeticOffset, nCosmeticOffset, width, height, NULL, NULL, GetModuleHandle(nullptr), this);
#else
        PixelEngine_hWnd = CreateWindowEx(dwExStyle, "PixelEngine_PIXEL_GAME_ENGINE", "", dwStyle,
            nCosmeticOffset, nCosmeticOffset, width, height, NULL, NULL, GetModuleHandle(nullptr), this);
#endif
        ...
        return PixelEngine_hWnd;
    }

    bool PixelGameEngine::PixelEngine_OpenGLCreate()
    {
        // Create Device Context
        glDeviceContext = GetDC(PixelEngine_hWnd);
    ...
    LRESULT CALLBACK PixelGameEngine::PixelEngine_WindowEvent(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
    {
        static PixelGameEngine* sge;
        switch (uMsg)
        {
        case WM_CREATE:        sge = (PixelGameEngine*)((LPCREATESTRUCT)lParam)->lpCreateParams;
        case WM_MOUSEMOVE:
        {
            ...
            sge->PixelEngine_UpdateMouse(ix, iy);
            return 0;
        }
        case WM_SIZE:
        {
            sge->PixelEngine_UpdateWindowSize(lParam & 0xFFFF, (lParam >> 16) & 0xFFFF);
            return 0;
        }
        case WM_MOUSEWHEEL:
        {
            sge->PixelEngine_UpdateMouseWheel(GET_WHEEL_DELTA_WPARAM(wParam));
            return 0;
        }
...
#if defined(__linux__)
    // Do the Linux stuff!
    Display* PixelGameEngine::PixelEngine_WindowCreate()
    {
        XInitThreads();

        // Grab the deafult display and window
        PixelEngine_Display = XOpenDisplay(NULL);
        PixelEngine_WindowRoot = DefaultRootWindow(PixelEngine_Display);

        // Based on the display capabilities, configure the appearance of the window
        GLint PixelEngine_GLAttribs[] = { GLX_RGBA, GLX_DEPTH_SIZE, 24, GLX_DOUBLEBUFFER, None };
        PixelEngine_VisualInfo = glXChooseVisual(PixelEngine_Display, 0, PixelEngine_GLAttribs);
        PixelEngine_ColourMap = XCreateColormap(PixelEngine_Display, PixelEngine_WindowRoot, PixelEngine_VisualInfo->visual, AllocNone);
        PixelEngine_SetWindowAttribs.colormap = PixelEngine_ColourMap;

        // Register which events we are interested in receiving
        PixelEngine_SetWindowAttribs.event_mask = ExposureMask | KeyPressMask | KeyReleaseMask | ButtonPressMask | ButtonReleaseMask | PointerMotionMask | FocusChangeMask | StructureNotifyMask;

        // Create the window
        PixelEngine_Window = XCreateWindow(PixelEngine_Display, PixelEngine_WindowRoot, 30, 30, nScreenWidth * nPixelWidth, nScreenHeight * nPixelHeight, 0, PixelEngine_VisualInfo->depth, InputOutput, PixelEngine_VisualInfo->visual, CWColormap | CWEventMask, &PixelEngine_SetWindowAttribs);

        Atom wmDelete = XInternAtom(PixelEngine_Display, "WM_DELETE_WINDOW", true);
        XSetWMProtocols(PixelEngine_Display, PixelEngine_Window, &wmDelete, 1);

        XMapWindow(PixelEngine_Display, PixelEngine_Window);
        XStoreName(PixelEngine_Display, PixelEngine_Window, "OneLoneCoder.com - Pixel Game Engine");

        if (bFullScreen) // Thanks DragonEye, again :D
        {
            Atom wm_state;
            Atom fullscreen;
            wm_state = XInternAtom(PixelEngine_Display, "_NET_WM_STATE", False);
            fullscreen = XInternAtom(PixelEngine_Display, "_NET_WM_STATE_FULLSCREEN", False);
            XEvent xev{ 0 };
            xev.type = ClientMessage;
            xev.xclient.window = PixelEngine_Window;
            ...
            XMapWindow(PixelEngine_Display, PixelEngine_Window);
            XSendEvent(PixelEngine_Display, DefaultRootWindow(PixelEngine_Display), False,
                SubstructureRedirectMask | SubstructureNotifyMask, &xev);
            XFlush(PixelEngine_Display);
            XWindowAttributes gwa;
            XGetWindowAttributes(PixelEngine_Display, PixelEngine_Window, &gwa);
            nWindowWidth = gwa.width;
            nWindowHeight = gwa.height;
        ...
        return PixelEngine_Display;
    }

    bool PixelGameEngine::PixelEngine_OpenGLCreate()
    {
        glDeviceContext = glXCreateContext(PixelEngine_Display, PixelEngine_VisualInfo, nullptr, GL_TRUE);
        glXMakeCurrent(PixelEngine_Display, PixelEngine_Window, glDeviceContext);
        ...
        if (glSwapIntervalEXT != nullptr && !bEnableVSYNC)
            glSwapIntervalEXT(PixelEngine_Display, PixelEngine_Window, 0);
        return true;
    }
    ...
    PixelEngine::PixelGameEngine* PixelEngine::PGEX::pge = nullptr;
#ifdef PixelEngine_DBG_OVERDRAW
    int PixelEngine::Sprite::nOverdrawCount = 0;
#endif
    //=============================================================
}
